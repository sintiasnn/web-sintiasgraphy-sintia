...
app.get("/api/experience", async (_req: Request, res: Response) => {
	try {
		...
		if (process.env.NODE_ENV !== "production")
			console.error("/api/experience error:", e);
...
app.get("/api/community", async (_req: Request, res: Response) => {
	try {
		...
		if (process.env.NODE_ENV !== "production")
			console.error("/api/community error:", e);
		return res.json({ items: [] });
	}
});
...
app.get("/api/projects", async (req: Request, res: Response) => {
	...
	const noCache =
		(req.query.noCache as string) === "1" ||
		(req.query.refresh as string) === "1";
	...
	try {
		...
		if (!noCache && cached && now - cached.at < GH_CACHE_TTL_MS) {
			...
			let filtered = (repos || []).filter((repo) =>
				includeForks ? true : !repo.fork,
			);
			if (featuredNames.length > 0) {
				const set = new Set(featuredNames);
				filtered = filtered.filter(
					(repo) =>
						set.has(repo.name.toLowerCase()) ||
						(repo.full_name && set.has(repo.full_name.toLowerCase())),
				);
				// preserve order according to featuredNames
				filtered.sort((a, b) => {
					const ia = featuredNames.indexOf(
						(a.full_name || a.name).toLowerCase(),
					);
					const ib = featuredNames.indexOf(
						(b.full_name || b.name).toLowerCase(),
					);
					return ia - ib;
				});
			}
			items = filtered.map((repo) => {
				...
				// Prefer GitHub repo URL for consistent "Lihat Repo" action
				...
				const updated =
					(repo.pushed_at || repo.updated_at || undefined) ?? undefined;
...
app.get("/api/medium", async (req: Request, res: Response) => {
	...
	const feedUrl =
		feedUrlEnv ||
		(username ? `https://medium.com/feed/@${username}` : undefined);
	...
	try {
			...
			try {
				...
				for (const block of xml.match(itemRegex) ?? []) {
					...
					const snippet = c
						...
						.trim();
					if (title && link)
						(items as any).push({ title, link, pubDate, image, snippet });
				}
			}
		...
		if (process.env.NODE_ENV !== "production")
			console.error("/api/medium error:", e);
