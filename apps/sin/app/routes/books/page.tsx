  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <ul className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((b) => (
          <li key={b.link} className="flex gap-3 rounded border p-3 dark:border-gray-800">
            {b.image ? (
              <img
                src={b.image}
                alt="cover"
            ...
            <div className="min-w-0">
              ...
              {b.pubDate && (
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{new Date(b.pubDate).toDateString()}</p>
              )}
            </div>
          </li>
        ))}
