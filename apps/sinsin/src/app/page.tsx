  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-10 min-h-[calc(100dvh-56px)] flex flex-col justify-center gap-8 md:gap-10 lg:gap-12">
      ...
      <section className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 justify-items-stretch auto-rows-fr">
        ...
        <a href="/blogs" className="block h-full rounded border p-4 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800/40">
                      ...
                      <li key={idx} className="rounded border p-3 sm:p-4 dark:border-gray-800">
                        <div className="flex items-baseline justify-between gap-3">
                          <div>
                            <div className="font-medium">
                              ...
                              {ex.company && (
                                <>
                                  ...
                                  {ex.companyUrl ? (
                                    <a href={ex.companyUrl} target="_blank" rel="noopener noreferrer" className="hover:underline">
                                      {ex.company}
                                    </a>
                                  ) : (
                            ...
                            {ex.type && <div className="text-xs text-gray-500 dark:text-gray-400">{ex.type}</div>}
                          </div>
                          {(start || end) && <div className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">{start} – {end}</div>}
                        </div>
                            ...
                            <div className="font-medium">
                              ...
                              {co.org && (
                                <>
                                  <span className="text-gray-500"> · </span>
                                  {co.orgUrl ? (
                                    <a href={co.orgUrl} target="_blank" rel="noopener noreferrer" className="hover:underline">
                                      {co.org}
                                    </a>
                                  ) : (
                                    <span>{co.org}</span>
