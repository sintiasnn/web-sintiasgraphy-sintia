  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <button
          onClick={async () => { await load(page, true); setShowRefreshed(true); setTimeout(() => setShowRefreshed(false), 1800) }}
      ...
      <ul className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {loading
          ...
          <li key={it.link} className="rounded border p-4 dark:border-gray-800">
            ...
            <div className="mt-3">
              <a href={it.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded border px-2 py-1 text-xs hover:bg-gray-50 dark:hover:bg-gray-800">
                View on Medium →
              </a>
            </div>
          </li>
        ...
        {items.length === 0 && !loading && (
          <li className="flex flex-col items-center justify-center gap-3 rounded border p-6 text-gray-600 dark:border-gray-800 dark:text-gray-400">
            ...
            <button onClick={() => load(page, true)} className="rounded border px-3 py-1 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">Retry</button>
          </li>
        )}
      </ul>
      {(pageCount && pageCount > 1) && (
        <div className="mt-8 flex items-center justify-between text-sm">
          {hasPrev ? (
            <button onClick={() => load(page - 1)} className="rounded border px-3 py-1 hover:bg-gray-50 dark:hover:bg-gray-800">← Newer</button>
          ) : <span />}
          <span className="text-gray-600 dark:text-gray-400">Page {page}{pageCount ? ` of ${pageCount}` : ''}</span>
          {hasNext ? (
            <button onClick={() => load(page + 1)} className="rounded border px-3 py-1 hover:bg-gray-50 dark:hover:bg-gray-800">Older →</button>
          ) : <span />}
        </div>
      )}
    </main>
