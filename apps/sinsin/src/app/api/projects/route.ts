...
export async function GET(req: NextRequest) {
  ...
  const includeForks = (searchParams.get('includeForks') as string) === '1'
  const noCache = (searchParams.get('noCache') as string) === '1' || (searchParams.get('refresh') as string) === '1'
  const namesParam = (searchParams.get('names') as string) || ''
  ...
  try {
    ...
    if (limit) return NextResponse.json({ items: items.slice(0, limit) })
